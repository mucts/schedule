...
module.exports = {
    ...
    async getList(context) {
        ...
        let queueInfo = await context.app.orm.sys.checkQueueInfo.first({id:queueId});
        if(!queueInfo){
            context.status = KoaConfig.constant.badRequest;
            context.badRequest("排班队列不存在");
            return;
        }
        if(queueInfo.supportAppointment !== 1){
            context.status = KoaConfig.constant.badRequest;
            context.badRequest("该排班队列不开放预约");
            return;
        }
        require("date-utils");
        ...
        if (typeof config === "object" && config.beginDay >= 0 && config.days > 0) {
            ...
            let rows = await context.app.orm.sys.checkScheduleConfig.find(`queue_id = ${queueId} AND is_delete = 0 AND (sc_type = 1 OR (sc_type= 2 AND sc_date BETWEEN '${beginDate}' AND '${endDate}'))`);
            let list = [];
            ...
            for (let i = config.beginDay; i < (config.beginDay + config.days); i++) {
                ...
                if (status === 1) {
                    ...
                    if (i === 0 && remaining) {
                        const result = await sc.getTimeSlot(context.app.orm, queueId, shiftDay, false);
    ...
    async getInfo(context) {
        ...
        let result = await sc.getTimeSlot(context.app.orm, queueId, shiftDate, false);
        context.status = KoaConfig.constant.success;
        context.body = {
            code: KoaConfig.constant.success,
            message: "success",
